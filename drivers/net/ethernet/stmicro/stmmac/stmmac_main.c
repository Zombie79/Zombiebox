...
#define EFUSE_MAC_UNUSED "/system/etc/efuse_mac_unused"

static bool Is_efuse_mac_unused(void)
{
	struct  file *file = NULL;

	file = filp_open(EFUSE_MAC_UNUSED, O_RDONLY,0);
	if (IS_ERR(file))
	{
		printk("open %s failed.", EFUSE_MAC_UNUSED);
		return 0;
	}else{
		printk("open %s suceed.", EFUSE_MAC_UNUSED);	
		return 1;
	}
	return 0;
}
...
#endif
		if (ret < 0) {
			err = eth_mac_efuse(dev->dev_addr);
			efuse_mac_unused = Is_efuse_mac_unused();
			if (err || ((dev->dev_addr[2] == 0) && (dev->dev_addr[3] == 0) && (dev->dev_addr[4] == 0)&& (dev->dev_addr[5] == 0)) || efuse_mac_unused) {
				...
				printk("eth mac right efuse***********:%X:%X:%X:%X:%X:%X\n",dev->dev_addr[0],
							dev->dev_addr[1],dev->dev_addr[2],dev->dev_addr[3],
							dev->dev_addr[4],dev->dev_addr[5] );
		}
...
static ssize_t eth_reset_func(struct device *dev, 
		struct device_attribute *attr,const char *buf, size_t count)
{
   	int res = 0;
	stmmac_release(my_ndev);
	res = stmmac_open(my_ndev);
	if (res != 0) {
...
static DEVICE_ATTR(eth_reset, 0666, NULL, eth_reset_func);
				     ...
				     void __iomem *addr)
{
	...
	ret = device_create_file(&priv->mii->phy_map[priv->plat->phy_addr]->dev,&dev_attr_eth_reset);
	if(ret < 0) {  
		printk("device_create_file	dev_attr_pwr_led_enable  fail!!!! \n");
	}

		     ...
		     unsigned int type, int event)
{
	...
	if (remote_mouse_event(dev, scancode, type, key_pointer_switch)) {
		...
		if (type == 2 && ((scancode == 0x1c) || (scancode == 0x4d) || (scancode == 0x40) || (scancode == 0x00)) &&
		...
		if (gp_remote->sleep && ((scancode == 0x1c) || (scancode == 0x4d) || (scancode == 0x40) || (scancode == 0x00)) &&
		    key_map[gp_remote->map_num][scancode] == 0x0074) {
...
static void remote_early_resume(struct early_suspend *handler)
{
	printk("xiezx adds remote_early_resume\n");
	//for A95X LED GPIODV_24
...
static int remote_probe(struct platform_device *pdev)
{
	...
	early_suspend.resume = remote_early_resume;
